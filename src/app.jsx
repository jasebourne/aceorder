...
const ItemInput = ({ item, quantity, onQuantityChange }) => {
    ...
    const itemTotal = (quantity * item.cost).toFixed(2);
    ...
    return (
        <div className="flex items-center space-x-2 p-2 bg-gray-700 rounded-lg shadow-inner">
            {/* Icon Block */}
            <div className={`p-1 w-10 h-10 md:w-12 md:h-12 flex-shrink-0 ${item.color} border-2 border-cyan-400 rounded-lg flex items-center justify-center shadow-lg`}>
            ...
            <div className="flex-grow flex items-center justify-between space-x-2">
                ...
                <input
                    ...
                    className="w-16 md:w-20 bg-gray-900 text-white text-center text-xl font-mono p-1 rounded-md border-2 border-gray-600 focus:border-cyan-500 transition duration-150"
                />
                {/* Individual Item Cost (Optional display for user clarity) */}
                {/* <span className="text-xs text-gray-400 hidden sm:block">({itemTotal} G)</span> */}
            </div>
        </div>
    );
};
...
export default function App() {
    const { userId } = useFirebaseSetup(); // Initialize Firebase boilerplate
    ...
    const totalGemCost = useMemo(() => {
        ...
        // Use toFixed(1) to match the image's format (700.3)
    ...
    return (
        <div className="min-h-screen bg-gray-800 flex flex-col items-center p-4 sm:p-8 font-sans">
            <header className="text-white mb-6 text-3xl font-bold border-b-4 border-cyan-400 pb-2">
                Gem Material Calculator
            </header>
            
            <p className="text-xs text-gray-400 mb-4">
                User ID (for reference): {userId || 'Loading...'}
            </p>

            <main className="w-full max-w-4xl bg-gray-900 p-4 sm:p-6 rounded-xl shadow-2xl">
                ...
                {/* Footer / Results Area */}
                <div className="mt-8 pt-4 border-t-2 border-gray-700 flex justify-between items-center flex-wrap gap-4">
                    
                    {/* Total Cost Display */}
                    <div className="flex items-center text-white text-xl md:text-2xl font-semibold bg-gray-700 p-3 rounded-lg shadow-lg">
                        <span className="mr-4 text-gray-300">Total Gem Cost:</span>
                        <span className="font-mono text-cyan-400 mr-2">{totalGemCost}</span>
                        <Gem className="w-5 h-5 text-green-400 fill-green-400" />
                    </div>

                    {/* Clear Button (XÓA) */}
                    <button
                        onClick={handleClear}
                        className="bg-yellow-500 hover:bg-yellow-600 active:bg-yellow-700 text-gray-900 font-bold py-3 px-6 rounded-lg shadow-md transition transform hover:scale-105 active:scale-95 text-xl tracking-widest uppercase"
            ...
            <footer className="mt-8 text-sm text-gray-500">
                *Item costs are placeholders. Edit the ITEM_DATA array in the code to set actual values.
            </footer>
        </div>
    );
}